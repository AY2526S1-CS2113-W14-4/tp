...
<li>The <strong>Nearest Deadline</strong> displays the internship with the closest upcoming deadline. If no future deadlines exist, it shows the most recent past deadline and marks it as <code>(OVERDUE!)</code>. It also shows the number of internships that share the same deadline (if any).</li>
<br>Example:
